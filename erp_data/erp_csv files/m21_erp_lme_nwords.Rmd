...
```{r}
cmplx <- n250_nwords_lme |> filter(complexity == "Complex")
smplx <- n250_nwords_lme |> filter(complexity == "Simple")
```

### Complexity by Language Type Model

```{r , comment=NA}

null.model <-  lmer(value_imp ~ 1 + laterality + anteriority +(1|SubjID) + (1|STIM), data = n250_nwords_lme,REML=FALSE)
summary(null.model)

complexity.langtype.model = lmer(value_imp ~ 1 +  complexity * lang_type + laterality + anteriority + (1|SubjID) + (1|STIM), 
                             data= n250_nwords_lme, REML=FALSE)
summary(complexity.langtype.model)

anova(null.model,complexity.langtype.model)
```



## ANOVA for Complex NonWords

### Null Model

```{r, comment=NA}

null.model <-  lmer(value_imp ~ 1 + laterality + anteriority +(1|SubjID) + (1|STIM), data = cmplx,REML=FALSE)
summary(null.model)
```
...
langtype.model <- lmer(value_imp ~ 1 + lang_type + laterality + anteriority +(1|SubjID) + (1|STIM), data = cmplx,REML=FALSE)
...
null.model <-  lmer(value_imp ~ 1 + laterality + anteriority +(1|SubjID) + (1|STIM), data = cmplx,REML=FALSE)
summary(null.model)
langtype.continuous.model <- lmer(value_imp ~ 1 + Dim.2 + laterality + anteriority +(1|SubjID) + (1|STIM), data = cmplx,REML=FALSE)
